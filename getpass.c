...
# endif
  ...
  /* Save old SIGINT handler, and set our own. */
  new_signal.sa_handler = sigint_handler;
  new_signal.sa_sigaction = NULL;
  new_signal.sa_flags = 0;
  sigemptyset (&new_signal.sa_mask);
  sigaction (SIGINT, &new_signal, &old_signal);
  /* TODO maybe we should pass our 'in' or 'out' to the signal handler? */
...
# endif
  ...
  sigaction (SIGINT, old_signal, NULL);
